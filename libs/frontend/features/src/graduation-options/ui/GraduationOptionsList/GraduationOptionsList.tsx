...
export function GraduationOptionsList() {
  ...
  const [selectedOption, setSelectedOption] = useState<GraduationOption | null>(
    null
  );
  ...
  const loadGraduationOptions = useCallback(async () => {
    const result = await listGraduationOptions({
      page,
      limit: 10,
    ...
    if (!result.success) {
      showToast({
        ...
        message:
          result.error || 'No se pudieron cargar las opciones de titulación',
      });
    }
  }, [
    page,
  ...
  const handleExportToExcel = useCallback(async () => {
    ...
    try {
      ...
      const exportColumns: TableColumn<GraduationOption>[] = [
        {
          key: 'name',
          label: 'Nombre',
        },
      ...
      const exportData = response.data.map((option) => ({
        ...option,
        isActive: option.isActive ? 'Activa' : 'Inactiva',
      }));

      // Generar nombre de archivo con fecha
      const dateStr = new Date().toISOString().split('T')[0];
      const filename = `opciones-titulacion-${dateStr}`;

      // Exportar
      await exportTable({
        filename,
        sheetName: 'Opciones de Titulación',
      ...
      showToast({
        ...
        message:
          'Las opciones de titulación se han exportado a Excel correctamente',
      });
    } catch (error) {
      console.error('Error al exportar opciones de titulación:', error);
      showToast({
        type: 'error',
        ...
        message:
          error instanceof Error
            ? error.message
            : 'No se pudo exportar las opciones de titulación',
      });
    } finally {
