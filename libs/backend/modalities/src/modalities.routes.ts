...
import type { ModalitiesController } from './modalities.controller.js';

export interface ModalitiesRouterDeps {
  getModalitiesController: () => ModalitiesController;
  requireAdmin: RequestHandler;
}

export function createModalitiesRouter(deps: ModalitiesRouterDeps): Router {
  const router = Router();
  const getController = deps.getModalitiesController;
