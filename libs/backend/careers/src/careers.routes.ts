...
import type { CareersController } from './careers.controller.js';

export interface CareersRouterDeps {
  getCareersController: () => CareersController;
  requireAdmin: RequestHandler;
}

export function createCareersRouter(deps: CareersRouterDeps): Router {
  const router = Router();
  const getController = deps.getCareersController;
